...
[![Cipher runtime](https://img.shields.io/badge/cipher--runtime-1.3.2%20(20260916c)-green?style=flat-square)](https://github.com/Oykdo/cipher/releases/tag/cipher-runtime-20260916c)
...
Cipher runtime `1.3.2` ([`cipher-runtime-20260916c`](https://github.com/Oykdo/cipher/releases/tag/cipher-runtime-20260916c)): [Windows x64](https://github.com/Oykdo/cipher/releases/download/cipher-runtime-20260916c/cipher-runtime.exe) · [Linux x86_64](https://github.com/Oykdo/cipher/releases/download/cipher-runtime-20260916c/cipher-runtime) · [SHA256SUMS](https://github.com/Oykdo/cipher/releases/download/cipher-runtime-20260916c/SHA256SUMS)
...
|---|---|---|---|
| **`v1.3.0`** | 2026-09-16 | Escrow Nexus (audit-hardened `escrow_7d`, real `OwnerSignature`, `check_release`, the Cipher bridge), genesis root 1 pinned in `config/genesis/`, the sphere custody ledger and client, the Eidos witness, batched finality, Connect escrow primitives, public-only packaging | **Source release**: the two vendored `eidolon_crypto` 0.1.0 wheels (`win_amd64`, `manylinux_2_34_x86_64`) + `SHA256SUMS`. No desktop build — use `v1.1.1`. The matching Cipher runtime is `cipher-runtime-20260916c` below. |
...
| [`cipher-runtime-20260916`](https://github.com/Oykdo/cipher/releases/tag/cipher-runtime-20260916) | `1.3.0` | private core after the genesis ceremony of 2026-09-16 (public tree `v1.3.0`) | Sphere custody client (`sphere list … trust`, `burn` / `reissue-key` / `queue`), **Escrow Nexus** (`escrow deposit … delete`), the genesis trust root compiled in (issuer `6dad3cb1…`, root `89fb26b1…`). Linux x86_64 (glibc ≥ 2.35) and Windows x64, `SHA256SUMS` alongside. Superseded the same day by `20260916b`. |
| [`cipher-runtime-20260916b`](https://github.com/Oykdo/cipher/releases/tag/cipher-runtime-20260916b) | `1.3.1` | private core after the genesis ceremony, one change over `1.3.0` (public tree `v1.3.0`) | Same surface as `20260916`, plus `sphere sync` names what the genesis treasury still holds for the vault (`claimable`) — a sphere without a head is in neither *final* nor *waiting*, so a client could not tell a claim was missing. Linux x86_64 (glibc ≥ 2.35) and Windows x64, `SHA256SUMS` alongside. |
| [`cipher-runtime-20260916c`](https://github.com/Oykdo/cipher/releases/tag/cipher-runtime-20260916c) | `1.3.2` | private core `d96fb3a`, one change over `1.3.1` (public tree `v1.3.0`) | Same surface as `20260916b`, plus every sphere status (`sphere list` / `sync` / `claim`) carries `visual` — theme, manifestation, essence and cosmic signature of the revealed template, nothing else of it — so Cipher's Spheres tab draws each sphere as a WebGL orb. Linux x86_64 (glibc ≥ 2.35) and Windows x64, `SHA256SUMS` alongside. Cipher's next release pins it. |
